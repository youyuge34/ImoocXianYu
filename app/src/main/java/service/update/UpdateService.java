...
public class UpdateService extends Service {
     ...
     * 外部存储位置判断是否可以写入
     */
    private boolean mCheckStorage = false;
    ...
    public void onCreate() {
        LogUtils.d("updateService","onCreate:");
        notificationManager = (NotificationManager) getSystemService(android.content.Context.NOTIFICATION_SERVICE);
        //先检查外部存储的当前状态，以判断是否可用
        mCheckStorage = isExternalStorageWritable();
        if(mCheckStorage){
            //地址设置为外部存储的私有目录：Android/data/com.xxx.xxx/files/update/，可以免去运行时权限
            filePath = getExternalFilesDir("update") + "/manyou.apk";
        }else {
            LogUtils.e("UpdateService","外部存储不可写入");
//            Toast.makeText(getApplicationContext(),"外部存储不可写入",Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void startDownload() {
        LogUtils.d("updateService","startDownload()");
        if(!mCheckStorage){
    ...
    private void notifyThatExceedLv21(String tickerMsg, String message, int progress) {
        ...
        notification.setSmallIcon(R.mipmap.manyou_logo);
        notification.setLargeIcon(BitmapFactory.decodeResource(getResources(),R.mipmap.manyou_logo));
    ...
    public boolean isExternalStorageWritable() {
        ...
        LogUtils.d("updateService","isExternalStorageWritable->state: "+state);
