...
import com.example.yousheng.imoocsdk.okhttp.HttpConstant;
...
/**
 * @author: qndroid
 * @function: 管理slot, 与外界进行通信，api层封装slot层
 * @date: 16/6/1
 */
public class VideoAdContext implements AdSDKSlotListener {
    ...
    private CostumeVideoView.ADFrameImageLoadListener mFrameLoadListener;

    public VideoAdContext(ViewGroup parentView, String instance,
                          CostumeVideoView.ADFrameImageLoadListener frameLoadListener) {
        this.mParentView = parentView;

        Gson gson = new Gson();
        this.mInstance =  gson.fromJson(instance,AdValue.class);
